...
// It is not legal scala code to have def's vals or vars at the top level, generated or not
// Thus all are commented out without mentioning why

// At the top level, the type of tree and name must be the same
//@toObject trait foo1
...
// At the top level, the name must be the same
...
// These do not work. It seems like something that would be desired though
